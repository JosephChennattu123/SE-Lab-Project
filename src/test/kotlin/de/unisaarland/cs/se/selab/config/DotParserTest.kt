...
class DotParserTest {
    ...
    fun parseEdges() {
        // val input = ";" + "1" + "\n" + "->" + "\n" + "2" + "\n" + "[label=" + "\n" + "test;" + "\n" + "labelb=text;" + "];"
        val input = """5 -> 6 [village=Saarland; name=Motorway; heightLimit=a; weight = 1.6; primaryType=countyRoad; secondaryType=none;];
    ...
    15 -> 1 [village=Saarland; name=Fastlane; heightLimit=10; weight = 29; primaryType=countyRoad; secondaryType=none;];"""
        ...
        val regex = """\s*[0-9]+\s*->\s*[0-9]+\s*\[(\s*[a-zA-z]+\s*=\s*[a-zA-z0-9.\d]+\s*;\s*)*\s*]\s*;""".toRegex()
